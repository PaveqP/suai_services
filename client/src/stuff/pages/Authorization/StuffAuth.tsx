...
import s from './StuffAuth.module.scss'
import { useSignInMutation } from "../../services/stuffAuthApi"
...
const StuffAuth = () => {
  ...
  const submitLoginForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      navigate(APP_ROUTES.stuffAccount)
    } catch (error) {
  ...
  return (
    <section className={s.authSection}>
      <div className={s.authModule}>
        <h1 className={s.title}>Вход для сотрудника</h1>
